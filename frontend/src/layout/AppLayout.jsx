...
const AppLayout = ({ children }) => {
  ...
  return (
    <div className="flex min-h-screen bg-background">
      <aside className={cn('fixed inset-y-0 z-40 w-64 border-r border-border bg-white p-5 transition-transform md:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')}>
        <div className="mb-8">
          <h1 className="text-xl font-bold">Semantic Admin</h1>
        ...
        <nav className="space-y-2">
          {navItems.map((item) => (
            <Link
              ...
              className={cn('block rounded-md px-3 py-2 text-sm font-medium', location.pathname === item.to ? 'bg-primary text-white' : 'text-slate-700 hover:bg-slate-100')}
            >
              {item.label}
            </Link>
          ))}
